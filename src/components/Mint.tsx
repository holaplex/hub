        ...
        <ul className="flex flex-col gap-2">
          {bullets.map((bullet, index) => (
            <li className="flex flex-col gap-1" key={index}>
              <span className="text-subtletext">{bullet.name}</span>
...
function MintSkeleton() {
  return (
    <Card>
      <div className="w-full aspect-square rounded-md bg-loadingui  animate-pulse" />
      <div className="flex justify-between mt-4">
        <ul className="flex flex-col gap-2">
          {Array.from(Array(2)).map((_, index) => (
            <li className="flex flex-col gap-1" key={index}>
              <span className="rounded-full h-4 w-14 bg-loadingui animate-pulse" />
