...
function WalletCard({ wallet, className }: WalletCardProps) {
  ...
  return (
    <Card className={clsx('p-6 items-center', className)}>
      ...
      <div className="text-xs font-medium text-subtletext mt-2">{wallet.assetId}</div>
      <div
...
function WalletSkeleton() {
  return (
    <Card className="p-6 items-center">
      <div className="rounded-full w-20 aspect-square bg-loadingui mt-2 animate-pulse" />
      <span className="rounded-full h-4 w-14 bg-loadingui animate-pulse mt-2" />
      <div className="flex gap-2 mt-2">
        <span className="rounded-full h-4 w-28 bg-loadingui animate-pulse" />
        <span className="rounded-md h-4 w-4 bg-loadingui animate-pulse" />
      </div>
    </Card>
  );
}
